...
$idArticulo=$_REQUEST['idArticulof'];
$descripcion=$_REQUEST['descripcionf'];
...
if(isset($lugar_entrega) && !empty($lugar_entrega) && isset($fecha_entrega) && !empty($fecha_entrega))
{
    ...
    mysql_query("INSERT INTO pedidos VALUES (DEFAULT,'$lugar_entrega','$fecha_entrega','P','$fecha','',$id,'','','','','')",$conex);
    ...
    for ( $i = 0 ; $i < $count ; $i++ )
    {
        if($cantidad[$i] !== "0" &&  !empty($cantidad[$i]))
        {
            ...
            $SQL = "INSERT INTO detalle_pedido VALUES (DEFAULT,'$IdPedido','$idArticulo[$i]','$precio[$i]','$monto','$cantidad[$i]','$fecha_entrega','$lugar_entrega','','','','','','','','','','')";
...
else
{
    echo '<script language = javascript>
                ...
                self.location = "realizar_pedido.php"
              </script>';
}
?>