...
$per_contacto=$_REQUEST['per_contacto'];
...
if($validate>=1 || $validate1>=1)
{
    echo '<script language = javascript>
                ...
                self.location = "registro_empresa.php"
              </script>';
}
else
{
    if(isset($sucursal) && !empty($sucursal) && isset($razon) && !empty($razon) && isset($per_contacto) && !empty($per_contacto) && isset($telefono_contacto) && !empty($telefono_contacto) && isset($email) && !empty($email)/* && isset($captcha) && !empty($captcha)*/)
    {
...
//        str_pad($value, 5, '0', STR_PAD_LEFT);
            ...
            mysql_query("INSERT INTO solicitudes_clientes VALUES (DEFAULT,'$sucursal','','','$telefono_contacto','$email','$celular','$razon','$per_contacto','$telefono','$fecha','P')",$conex);
